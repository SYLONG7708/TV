...
2026-09-10 修正：舊版直播資料更新成功後，仍會因 GitHub Pages 約 3.56 GB 的部署包超過 1 GB 上限而逾時。新版 Pages 只發布網頁、必要目錄和清單，設 100 MiB 硬上限；大型索引完整保留在 `gh-pages` 資料分支，瀏覽器按需讀取固定 commit 的原始檔案。網址與已安裝 APK 的來源設定維持相容。
...
- `deploy-oktv-pages.yml`：只下載必要檔案；目錄及大型索引鎖定同一資料 commit；檢查總量、直播陣列、CSP 與包大小；部署第一次失敗後等待 30 秒重試，完成後檢查實際上線版本及每個來源索引。
...
已將倉庫 Pages 的 publishing source 改為 GitHub Actions，並允許 `main` 分支部署到 `github-pages` environment。2026-09-10 首次新版部署與直播更新均已成功，實際部署包為 18,032,831 bytes；後續更新由雲端完成。
